...
export function PermissionsMatrix({ organizationId }: { organizationId?: string }) {
  const { t } = useTranslation();
  const qc = useQueryClient();
  const fetchMatrix = useServerFn(getOrgPermissionMatrix);

  const matrix = useQuery({
    queryKey: ["permission-matrix", organizationId ?? "self"],
    queryFn: () => fetchMatrix({ data: organizationId ? { organizationId } : {} }),
  });
  ...
  if (matrix.isLoading) return <div className="text-sm text-muted-foreground">{t("common.loading")}</div>;
  if (matrix.error) return <div className="text-sm text-destructive">{(matrix.error as Error).message}</div>;
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        {t("permissions.ownerNote", "Owners always have all permissions and cannot be restricted.")}
      </p>
    </div>
  );
}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {ALL_PERMISSIONS.map((perm) => {
              ...
              const fromRole = DEFAULT_ROLE_PERMISSIONS[memberRole]?.includes(perm) ?? false;
